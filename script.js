...
buttonDate.addEventListener("click", () => {  // Agregamos un evento al botón para que al hacer click se ejecute la función. Este evento cumple la función de un bucle, la cual permite que el usuario ingrese un número y lo compare con el número secreto, hasta que el usaurio lo adivine.
    ...
    if (numIntentos > 0)
        if (valorDato < numSecret) { //Indicamos si el valor ingresado por el usuario es mayor al número secreto, mostramos un mensaje de error, y se le descuenta un intento.
            resultado.textContent = `Ups, el número secreto es incorrecto, vuelve a intentarlo. Te quedan ${numIntentos} intentos.`;
            resultado.style.color = "red";
        } else { //Indicamos si el valor ingresado por el usuario es menor al número secreto, mostramos un mensaje de error, y se le descuenta un intento.
            resultado.textContent = `Ups, el número secreto es incorrecto, vuelve a intentarlo. Te quedan ${numIntentos} intentos.`;
            resultado.style.color = "red";
